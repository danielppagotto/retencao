...
retencao_medicos <- read_xlsx("C:/Users/alefs/OneDrive/Documentos/LAPEI-CIGETS/GitHub/retencao/0_dados/retencao_medicos_erika.xlsx") |> 
...
data_medicos <- hierarquia_atualizada %>% 
...
cloropetico <- spdf_fortified |>
  left_join(data_medicos, by = c("reg_id" = "cod_regsaud")) %>% 
  rename(Retenção = retencao) |> 
  ggplot() +
...
ggsave(cloropetico, filename = "cloropetico_medicos_erika_4.svg",
