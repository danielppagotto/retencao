...
retencao_psiquiatras <- read.csv("C:/Users/alefs/OneDrive/Documentos/LAPEI-CIGETS/GitHub/retencao/0_dados/Psiquiatra_retencao_geral.csv",
                                 sep = ";") %>% 
  select(-X)
...
data_psiquiatras <- hierarquia_atualizada %>% 
  left_join(retencao_psiquiatras, by = c("cod_regsaud" = "regiao_saude")) %>% 
  mutate(retencao_geral = if_else(is.na(retencao_geral), 0, retencao_geral))

ranking_10maiores <- data_psiquiatras |> 
...
write_xlsx(ranking_10maiores, 
          "C:/Users/alefs/OneDrive/Documentos/LAPEI-CIGETS/GitHub/retencao/0_dados/ranking_10maiores_psiquiatra.xlsx")
...
cloropetico <- spdf_fortified |>
  left_join(data_psiquiatras, by = c("reg_id" = "cod_regsaud")) %>% 
...
ggsave(cloropetico, filename = "cloropetico_retencao_psiquiatra.svg",
