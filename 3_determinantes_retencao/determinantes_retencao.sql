...
SELECT  c.uf_sigla,
        ...
        n.obitos_homicidios,
        (CAST(n.obitos_homicidios AS FLOAT)/CAST(b.pop_censo22 AS FLOAT)) * 10000 AS obitos_10000_hab,
        o.vinc_medio,
        p.PIB_1000,
    ...
    retencao_medico
     AS(
        SELECT  
            regiao_saude,  
            ROUND(retencao_geral,4) AS retencao
        FROM Dados.retencao."Médico_retencao_geral.parquet"
...
consulta_ies 
    AS(
        SELECT cod_regsaud, 
               regiao_saude,
               COUNT(DISTINCT(chave_ies)) AS total_ies_medicina
        FROM "Open Analytics Layer"."Educação"."Quantidade de vagas, matriculados, concluintes e inscritos em curso superior por instituição de ensino"
        WHERE ano = 2022 and curso = 'Medicina'
        GROUP BY cod_regsaud, 
                 regiao_saude
), 
...
SELECT  c.uf_sigla,
        ...
        b.pop_censo22,
        i.TOTAL_PROF,
        ROUND(CAST(i.TOTAL_PROF AS FLOAT)/CAST(b.pop_censo22 AS FLOAT) * 1000, 4) AS razao_medicos_1000,
        c.internacoes_sensiveis_aps,
        d.total_ies_medicina,
        e.matriculas_ies,
        e.ingressos_ies,
        e.vagas_ies,
        e.inscritos_ies,
    ...
    retencao_dentista
     AS(
        SELECT  
            regiao_saude,  
            ROUND(retencao_geral,4) AS retencao
        FROM Dados.retencao."Cirurgiões-dentistas_retencao_geral.parquet"
...
consulta_ies 
    AS(
        SELECT cod_regsaud, 
               regiao_saude,
               COUNT(DISTINCT(chave_ies)) AS total_ies_odonto
        FROM "Open Analytics Layer"."Educação"."Quantidade de vagas, matriculados, concluintes e inscritos em curso superior por instituição de ensino"
        WHERE ano = 2022 and curso = 'Odontologia'
        GROUP BY cod_regsaud, 
                 regiao_saude
), 
...
SELECT  c.uf_sigla,
        ...
        b.pop_censo22,
        i.TOTAL_PROF,
        ROUND(CAST(i.TOTAL_PROF AS FLOAT)/CAST(b.pop_censo22 AS FLOAT) * 1000, 4) AS razao_dentistas_1000,
        c.internacoes_sensiveis_aps,
        d.total_ies_odonto,
        e.matriculas_ies,
        e.ingressos_ies,
